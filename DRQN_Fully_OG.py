...
def train(
    ...
    q_target, _, _ = target_q_net(
        next_observations, h_target.to(device), c_target.to(device)
    )
...
if __name__ == "__main__":
    ...
    # Env parameters
    model_name = "DRQN_POMDP_Random_Fully"
    ...
    for i in range(episodes):
        ...
        if i % print_per_iter == 0 and i != 0:
            ...
            if score_sum / print_per_iter > max_score:
                print(
                    f"Saving model...score: {score_sum / print_per_iter} > max score: {max_score}"
                )
                max_score = score_sum / print_per_iter
